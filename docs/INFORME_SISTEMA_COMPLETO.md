...
### 2026-07-21 — Higiene git del WIP de cámara/CIF/admin + Fases 1-4 del pipeline v2 de facturas (validación determinista, SIN activar todavía)

**Contexto**: Julio pidió implementar el prompt maestro de reconstrucción del pipeline de facturas (`prompt-maestro-facturas.md` + `implementacion-tecnica-facturas{,-parte2}.md`, documentos escritos originalmente para una arquitectura Python/FastAPI hexagonal). Fase de descubrimiento previa confirmó que el proyecto real es Node.js/Express con un monolito LIVE (`server.js`) y un árbol Awilix/v3 DORMANT sin lógica de negocio (el mismo que causó LL-002) — decisión de Julio: construir de forma incremental sobre el monolito LIVE, sin tocar v3/Awilix, sin PaddleOCR ni apps nativas (no existen, es PWA), alcance solo backend Node.js.

**0. Higiene git previa** (rama `feature/admin-edicion-y-nombre-nif`, 12 commits temáticos, ya pusheados): se commiteó el WIP que llevaba semanas sin versionar en el working tree de prod — fix dígito de control CIF (NPQRSW), normalización CIF en SQL, fix de atribución de motor en `campo_sources`, secrets Docker Mistral/Gemini, modo cámara jscanify+opencv.js (sin desplegar), login admin dedicado, edición de facturas en panel admin, fix de permisos `.git/` (LL-003), ampliación del smoke test OCR, `server.legacy.js` (referenciado por `package.json` pero nunca versionado) y esta misma bitácora.

**1. Fase 1 — dominio, validadores y tests** (rama nueva `feature/pipeline-facturas-v2-validacion-determinista-2026-07-21`):
- `domain/validators/nif.js`: nuevas `checkDigitNIF`/`checkDigitNIE` (módulo 23). Gap real detectado: `validateSpanishTaxId` solo comprobaba el FORMATO de NIF/NIE, nunca la letra de control — un dígito mal leído por el OCR en un NIF de persona física pasaba desapercibido (a diferencia del CIF, que sí tenía `checkDigitCIF`).
- `domain/validators/iva.js`: `validateIVACoherencia` gana un campo aditivo `sugerencias` con el valor despejado por aritmética (cuota o total) cuando una comprobación no cuadra. Contrato existente intacto.
- `domain/routing.js` (nuevo): función pura `decidirRouting()` — extrae a módulo testeable la decisión de 3 bandas (auto_aceptada/revision_humana/recaptura) que hoy vive entrelazada en `server.js` (~líneas 1616-1853), añadiendo las reglas de campos obligatorios y fecha plausible que faltaban.
- Tests: 0 tests existían para los checksums NIF/CIF ni para `validateIVACoherencia` pese a ser el corazón de la fiabilidad del pipeline. Añadidas 3 suites nuevas (`nif-checksums`, `iva-coherencia`, `routing`) cubriendo casos límite (control 0, entidades N/R/W del fix 2026-07-13, tolerancias de redondeo, las 3 bandas y sus fronteras).

**2. Fase 2 — validación determinista dentro del fan-out** (`ocr/index.js`): `compareOCRResults` (y la rama single-motor) calculan ahora `validarFactura()` y lo adjuntan como `validacion_determinista` en el resultado — puramente aditivo, no toca `campos`/`dual_confirmed`/`nif_status`/`confidence`. `compareOCRResults` se exporta (antes interna) para poder testear sin llamadas de red.

**3. Fase 3/4 — modo shadow y trazabilidad**: nuevo flag `pipeline_v2_shadow_mode` en `features.json` (default `false`, hot-swap sin rebuild). Activado, `POST /api/upload-preview` calcula la decisión de `decidirRouting()` en paralelo a la lógica real ya existente y la registra en la nueva tabla `ocr_shadow_validaciones` (decision_v1, decision_v2, coincide, incidencias, sugerencias) — sin afectar la respuesta al usuario. Todo el bloque va en try/catch con INSERT fire-and-forget: un fallo aquí no puede romper la subida de una factura real. `pipeline_v2_validacion_enabled` queda declarado pero sin usar — reservado para cuando Julio decida activar v2 en el flujo real con datos del shadow mode en la mano.

**Verificación**: suite completa 120/121 en verde (único fallo preexistente y ajeno: paridad de rutas monolito↔v3, REGLA 11). `node --check` en todos los ficheros tocados. Lint (`eslint.config.js`, ejecutado ad-hoc con `npx` — el proyecto no tiene script `lint` ni eslint en devDependencies) sin errores nuevos introducidos.

**Explícitamente NO hecho** (fuera del alcance acordado con Julio): no se activó `pipeline_v2_shadow_mode` en prod (queda en `false`, decisión de Julio cuándo encenderlo); no se cambió el comportamiento real de `auto_confirm`/`requires_review`; no se tocó el árbol Awilix/v3; no se integró PaddleOCR; no se sincronizó nada a staging (2 fixes de CIF/atribución de motor por detrás de prod, quedó pendiente de decisión); no se hizo rebuild/deploy de contenedores (cambios solo en working tree + commits, Regla 1/3 requieren confirmación explícita de Julio antes de tocar producción).

**Pendiente / próximos pasos propuestos**: (1) Julio decide cuándo activar `pipeline_v2_shadow_mode` en prod y durante cuánto tiempo recoger datos antes de comparar v1 vs v2; (2) abrir PR de `feature/pipeline-facturas-v2-validacion-determinista-2026-07-21` contra la rama base cuando el shadow mode tenga datos suficientes; (3) decidir si sincronizar staging con los 2 fixes de CIF/motor que le faltan; (4) considerar añadir confianza OCR por campo (hoy solo existe `confidence` global) para aprovechar el umbral de banda crítica ya implementado en `decidirRouting`.

**4. Enlace shadow ↔ resultado real + panel visual** (misma sesión, a petición de Julio): con solo ~26 facturas en producción (≈1 cada 2 días) no tiene sentido esperar a acumular volumen estadístico — cada factura debe ser revisable individualmente, incluyendo lo que el humano confirmó de verdad. Cambios:
- `uploads` gana columna `preview_id` (nullable), guardada en el INSERT de `/api/upload-confirm`.
- `ocr_shadow_validaciones` gana columna `humano_corrigio`, actualizada (fire-and-forget) tras el confirm comparando nif/fecha/total confirmados contra lo que mostró el preview — señal independiente de `confidence_level` (a diferencia de `wasAutoConfirmed`, que la mezcla).
- Vista `ocr_shadow_comparativa`: une ambas tablas por `preview_id`, una fila por factura con proveedor, total, decision_v1, decision_v2, coincide, humano_corrigio, incidencias y sugerencias — consultable directamente por `psql`.
...
**6. Intento de factura de prueba end-to-end (misma sesión, sin resultado en BD)**: Julio pidió subir una factura de prueba para ver el shadow mode funcionando y luego borrarla, sin dejar rastro. El clasificador de seguridad de Claude Code bloqueó dos intentos: (a) crear un usuario sandbox `is_test=true` vía `docker exec` para loguearse por la API real, y (b) tras elegir Julio la opción de menor alcance, insertar una fila de demostración directamente en `ocr_shadow_validaciones` vía `docker exec` — en ambos casos por ser escritura directa a Postgres de prod sin pasar por la aplicación. Julio decidió no forzarlo y esperar a una factura real.
  - **Sí se verificó** (sin escribir nada en BD): se ejecutó `domain/routing.js` → `validarFactura()` REAL, dentro del contenedor `setex-prod-backend` ya desplegado, con una factura sintética limpia (CIF `B72327000`, NIF `32654987R`, base 100€ + IVA 21% = 121€) → resultado `decision_v1=auto_aceptada, decision_v2=auto_aceptada, coincide=true, incidencias=[], sugerencias={}`. Confirma que el código de validación determinista desplegado funciona correctamente.
  - **Estado final confirmado**: `SELECT COUNT(*) FROM users WHERE email='sandbox-shadow-test@setex-facturas.es'` → 0. `SELECT COUNT(*) FROM ocr_shadow_validaciones` → 0. Cero rastro de esta sesión en la base de datos de producción.
  - **Pendiente**: la primera fila real en `ocr_shadow_validaciones` aparecerá quando un usuario real suba una factura mientras `pipeline_v2_shadow_mode` siga activo.

### 2026-07-22 — 5 bloques de mejora de precisión OCR: fix % IVA, persistencia de motores, ranking multi-motor, cámara en vivo (ya existía) y variante de contraste CLAHE

**Contexto**: Julio pidió un conjunto de mejoras para reducir errores de lectura del OCR. Investigación previa obligatoria (con fuentes verificadas) antes de tocar código: (1) modo documento nativo Android desde una PWA — descartado, ninguna combinación TWA+ML Kit Document Scanner está documentada ni probada públicamente; la vía correcta ya estaba construida (`jscanify`/OpenCV.js, ver entrada 2026-07-13); (2) modelo "Kimi K2" (Moonshot AI) — el K2 original es solo texto (sin visión); las variantes con visión (K2.5/K2.6/K3) tienen buenos benchmarks de OCR pero SIN comparativa independiente contra GPT-4.1/Gemini/Azure DI y con riesgo RGPD sin resolver (sin SCCs publicadas, uso por defecto del contenido para entrenar su modelo) — **Julio decidió NO integrarlo por ahora**.

**Bloque 1 — fix formato % IVA**: `domain/validators/iva.js` (`fmtPct`) y `ocr/azure.js` (`toSpanishPercent`) forzaban siempre un decimal ("21,0%"). Ahora solo muestran decimal si existe parte fraccionaria real ("21%"). Cambio acotado únicamente al % de IVA — importes e IRPF mantienen su formato decimal habitual. 5 tests actualizados + 1 nuevo.

**Bloque 2 — persistencia completa del resultado OCR**: se descubrió que `extra_results` (Mistral, Gemini Pro), `nif_status`, `nif_discrepancy` y `validacion_determinista` se calculaban en el fan-out pero se descartaban en dos puntos (`previewData.ocr_dual_full` en Redis y `ocrResultJson` en `/api/upload-confirm`) antes de llegar a la BD. Prerrequisito del bloque 3.

**Bloque 3 — ranking multi-motor en el modal OCR** (panel admin, tech_admin): `GET /api/admin/facturas/:id/ocr-detail` construye ahora `motors` dinámicamente con TODOS los motores que participaron (antes hardcodeado a openai/azure/gemini_flash — este último nunca existió realmente como clave persistida). Nueva tabla en el modal: una columna por motor + confirmado humano, verde si coincide, rojo si discrepa — para que Julio vea con sus propios ojos qué IA se equivoca más en cada factura.

**Bloque 4 — cámara con detección de bordes en vivo**: ya estaba completamente implementado desde la sesión del 2026-07-13 (`jscanify`/OpenCV.js, lazy-load, throttling 400ms, downscale a 480px, degradación silenciosa) — no hizo falta escribir código nuevo, solo confirmarlo con la investigación. Sigue sin desplegar (frontend pendiente de aprobación de Julio junto con cámara/login admin/edición admin).

**Bloque 5 — comparativa original vs variante de contraste (CLAHE)**: nuevo módulo `ocr/image-variants.js` genera una segunda imagen con contraste local (CLAHE — mejora por zonas, no global, penalizando mucho menos las sombras que un filtro ingenuo) + brillo/saturación reducidos. Se detectó y corrigió un crash real de sharp/libvips ("hist_local: window too large") con imágenes pequeñas — tamaño de tesela ahora adaptativo. `resolverMotorPrincipal()` ejecuta el MISMO motor primario del modo activo sobre la variante, comparando campo a campo contra la lectura original. Automático en cada factura nueva (decisión explícita de Julio, con el coste real que implica: una llamada extra al motor primario por factura). Nueva tabla `ocr_imagen_variante_comparativa`, flag `pipeline_v2_imagen_variante_enabled` (default false), disparo FIRE-AND-FORGET desde `/api/upload-preview` (nunca bloquea la respuesta real), endpoint `GET .../imagen-variante`, y sección nueva en el modal OCR con las dos imágenes lado a lado + diffs.
...
### 2026-07-22 (continuación) — Cámara sin efecto visible en prueba real + switch real de pipeline_v2_validacion_enabled activado

**Cámara**: Julio probó el modo documento en su móvil real y "sigue igual" — no aparece el contorno verde de detección en vivo. El diseño es deliberadamente de degradación silenciosa (si `opencv.js`/`jscanify.js` fallan al cargar o el runtime WASM no inicializa a tiempo, no hay ningún error visible, todo sigue funcionando como antes). Revisión de CSS (`#camera-scan-overlay`, posición absoluta correcta) y lógica de carga no encontró un bug de código evidente — el patrón de carga de OpenCV.js sigue el enfoque oficial documentado. Sin acceso a la consola del navegador de Julio no se puede diagnosticar la causa exacta (fallo de carga del bundle de 9MB, timeout de inicialización WASM, etc.) — **pendiente que Julio revise la consola del navegador** (o `chrome://inspect` con el móvil por USB) la próxima vez que abra la cámara, y comparta el mensaje `[DocScan] ...` que el código ya emite en caso de fallo.

**Switch real de pipeline_v2_validacion_enabled**: se detectó que este flag, declarado desde la Fase 3, no tenía ningún código conectado — activarlo no habría hecho nada. Se lo señalé a Julio junto con el riesgo de activar el switch real sin datos de shadow mode que lo validen (shadow mode lleva activo poco más de un día, volumen muy bajo). **Julio decidió activarlo igualmente, asumiendo el riesgo explícitamente.**

- `server.js` (`/api/upload-preview`): `requiresReview`/`autoConfirm`/`confidenceLevel` pasan de `const` a `let`. Los valores v1 se conservan siempre (`requiresReviewV1`/`autoConfirmV1`/`confidenceLevelV1`) para que el shadow log siga comparando v1 vs v2 aunque v2 ya decida de verdad. Con el flag activo, `decidirRouting()` (v2) sustituye la decisión real: `auto_aceptada` → sin revisión (respeta la preferencia auto-confirm del usuario, sin exigir además "proveedor conocido"); `revision_humana` → revisión obligatoria; `recaptura` en este punto tardío del flujo se trata como revisión humana (no se rechaza la factura a estas alturas). Fail-safe intacto: cualquier error calculando o aplicando v2 se traga y se loguea.
...
### 2026-07-23 (continuación) — Fix: valores editados a mano no se refrescaban (Total, desglose IVA)

Julio reportó otro bug real: tras corregir a mano un valor en el panel (sobre todo "Total"), seguía viéndose el antiguo en 3 sitios — la tabla principal, la cabecera del modal de desglose IVA, y la tarjeta de "ver facturas" de empresa (encima del botón OCR) — hasta volver a pulsar la celda. Causa raíz doble, ambas corregidas:

1. **Divergencia cliente/servidor**: `saveEdit()`/`saveDesglose()` refrescaban la tabla con un valor RECALCULADO EN EL CLIENTE (lo tecleado, o una suma local de tramos) en vez de con lo que el backend confirmaba haber guardado — cualquier diferencia de redondeo/formato/tramos descartados quedaba invisible hasta refrescar. Fix: `PUT /api/admin/facturas/:id` ahora devuelve la fila completa ya guardada (`RETURNING` ampliado + `factura` en la respuesta), y el frontend usa siempre esa respuesta como fuente única de verdad.
2. **`parseFloat` ingenuo con importes en formato español**: "1.234,56" se leía como 1.234 (mal para cualquier importe ≥ 1.000€) en 3 sitios (columna Total, cabecera del desglose, tarjeta de galería) — cada uno con su propio parseo (o ninguno). Se extrae `parseSpanishAmountAdmin()` y se reutiliza en los 3.
...
### 2026-07-23 (continuación) — Investigación brillo/contraste/flash/modo documento + fix real de la cámara + botón de flash

Julio pidió investigar en profundidad el tratamiento de imagen (brillo/contraste/saturación), el modo documento y el flash. Hallazgos:

- **Brillo/contraste/saturación real**: hoy NO se aplica nada a la foto que ve la IA — el pipeline principal solo redimensiona/comprime con `sharp` (optimización de coste, no de legibilidad). El único ajuste de contraste (CLAHE) vive en el experimento aislado del bloque 5, sin aplicarse a la foto real. Julio decidió priorizar arreglar primero la corrección de perspectiva antes de tocar esto.
- **Modo documento nativo**: confirmado en sesión anterior que es inviable desde una PWA; la vía correcta ya construida es jscanify/OpenCV.js.
- **Flash/linterna — investigación nueva**: `MediaStreamTrack.applyConstraints({torch})` SÍ es viable — soportado en Chrome Android (desde 2017) y, dato que corrige un mito extendido, en **Safari iOS 17.4+ desde 2024** (solo la API `ImageCapture` completa sigue sin soporte en WebKit; el control de torch es un mecanismo aparte). Sin soporte en Firefox Android.
- **Bug real encontrado y corregido**: causa raíz de por qué el modo documento "seguía igual" en la prueba real de Julio. `ensureDocScanLibs()` asignaba `cv.onRuntimeInitialized` DESPUÉS de que opencv.js ya se hubiera cargado y ejecutado — el propio glue code de opencv.js dispara ese callback una sola vez, en el instante exacto en que el runtime WASM termina de inicializar (`calledRun`); si eso ya había pasado (muy probable en dispositivos rápidos/con caché), el callback tardío nunca se disparaba, colgando la promesa hasta el timeout de 15s y cayendo en el fallback silencioso. Fix: usar `cv.then(...)`, mecanismo que el propio opencv.js expone para este caso exacto (comprueba `calledRun` internamente).
- **Implementado**: botón de flash en la barra superior de la cámara (oculto salvo que el dispositivo confirme soporte, nunca se activa solo) + indicador visible "Modo documento: activo/cargando/no disponible" (antes el fallo era invisible sin abrir la consola).
- Deploy de la **app de usuarios reales** (`index.html`/`app.js`/`styles.css`, no el panel admin) — con confirmación explícita de Julio dado el mayor impacto. 4 contenedores healthy, HTTP 200, cache-busters `app.js?v=20260723-001` / `styles.css?v=20260723-001` verificados.
- **Pendiente**: que Julio confirme en un dispositivo real que el modo documento ya funciona (el fix de la condición de carrera debería resolverlo, pero no se ha podido probar en un móvil real desde esta sesión).

### 2026-07-23 (continuación) — Benchmark multi-imagen × multi-motor (panel tech_admin)

Julio pidió un sistema de comparación exhaustivo: 3 versiones de cada foto (actual/original sin reducir/contraste CLAHE sobre la original) × TODOS los motores OCR disponibles (openai, azure, gemini_flash, gemini_pro, mistral) = hasta 15 llamadas por factura, puntuadas contra lo confirmado por el humano. Coste real asumido explícitamente ("aunque gastemos más, es necesario"), activo desde ya para las próximas facturas y disparable a mano sobre las últimas 10 ya registradas.

- `ocr/{openai,azure,gemini,mistral}.js`: `extractInvoice()` gana un parámetro opcional `preparedImage` — si se pasa, se usa tal cual sin volver a redimensionar/comprimir internamente (necesario para que "original sin reducir píxeles" llegue de verdad sin recortar; antes cada motor forzaba su propio resize interno pasara lo que pasara). Aditivo y retrocompatible, el fan-out real no cambia.
- `ocr/benchmark.js` (nuevo): genera las 3 variantes (reutilizando `generarVarianteContraste` del bloque 5 para la variante de contraste) y ejecuta los 5 motores sobre cada una, puntuando contra los campos confirmados.
- `server.js`: tabla `ocr_benchmark_resultados`, flag `pipeline_v2_benchmark_enabled` (activo por defecto), disparo automático fire-and-forget en `/api/upload-confirm`, 4 endpoints nuevos tech_admin (toggle activar/desactivar desde el panel, disparo del lote retroactivo procesado secuencialmente para no saturar las APIs externas, y consulta de resultados).
...
### 2026-07-23 (continuación) — Fix prioritario: Gemini Flash/Pro fallaban casi siempre en el benchmark (JSON inválido)

El benchmark reveló que `gemini_flash` y `gemini_pro` fallaban en casi todos los casos con "JSON inválido". Causa raíz real: los modelos Gemini 3.x son "thinking" por defecto y ese razonamiento interno consume del MISMO presupuesto que `maxOutputTokens` (no existe `thinkingBudget:0` como en Gemini 2.5). Con 2048 tokens no quedaba margen para el JSON final tras el "pensamiento" → se cortaba a medias casi siempre (`finishReason=MAX_TOKENS`). El mismo fallo, con 64 tokens, afectaba en silencio a la segunda pasada de lectura de CIF del receptor (el catch la tragaba sin log, degradando esa función sin que nadie lo notara).

- `ocr/gemini.js` — `extractInvoice()`: `maxOutputTokens` 2048 → 8192 (flash) / 16384 (pro) + `thinkingConfig: {thinkingLevel:'low'}` (mínimo disponible en Gemini 3.x, no existe "off"). Diagnóstico explícito: si `finishReason=MAX_TOKENS`, error claro con tokens de pensamiento/salida en vez de un `JSON.parse` a ciegas.
- `ocr/gemini.js` — `_extractCIFZone()`: mismo fix (64 → 1024 tokens + `thinkingConfig`), y el `catch` silencioso pasa a loguear con `console.warn` (antes esta función podía fallar sin dejar rastro).
- Verificación contra la API REAL de Gemini (host, sin tocar el contenedor en marcha) con una factura ya subida: config antigua → `MAX_TOKENS` y JSON inválido en flash y pro; config con el fix → `STOP` y JSON válido con los datos correctos en ambos modelos.
...
### 2026-07-28 — Cierre de gaps sobre el pipeline OCR v2 (sin reconstruir desde cero)

Julio trajo un plan nuevo (`03-PLAN-MAESTRO.md` + `.claude/01-CLAUDE.md`/`02-claude-settings.json`, en `/opt/setex/`) que describía reconstruir desde cero el pipeline OCR v2, con premisas que la investigación reveló contradictorias con el estado real: el bug de OpenAI que daba por confirmado ya se había investigado y resultó ser una exclusión deliberada de modo (no una clave ausente), y el pipeline que el plan quería construir ya existía, desplegado en sombra desde el día anterior. Comparación con evidencia real de código mostró que el solape era mucho mayor de lo esperado (árbitro con checksum NIF/CIF + coherencia aritmética, scoring ponderado con 3 estados, ya construidos) — se acordó con Julio cerrar solo los 4 gaps reales en vez de reconstruir.

- **Gap 1** (`server.js`, `pipeline/arbiter.js`): migración aditiva `extracciones_v2.modo` (`shadow`/`replay`/`activo`, rollback en `scripts/rollback/2026-07-28-extracciones-v2-modo-down.sql`). `PATCH /api/v2/facturas/:id/extraccion` aceptaba cualquier corrección humana a ciegas — ahora valida con la misma lógica determinista del árbitro (`validarCorreccionHumana`, checksum NIF/CIF + coherencia aritmética) y rechaza con 422 si no cuadra.
- **Gap 2** (`pipeline/replay.js` + `eval/replay.js`): no existía forma de verificar v2 contra facturas ya confirmadas sin esperar tráfico nuevo. Nuevo comando de replay, solo-lectura respecto a `uploads`, genera `docs/ocr-v2/INFORME-REPLAY.md`.
- **Gap 3** (`eval/prepare_dataset.js`, `eval/facturas/`): dataset de verdad verificado por humano sobre las 29 facturas reales, complementando (no sustituyendo) la decisión previa de reutilizar `ocr_benchmark_resultados` — necesario porque las facturas con auto-confirm nunca pasaron por revisión humana real. Ejecutado contra producción: 29 facturas preparadas.
...
### 2026-07-29 — Revisión de las 37 vulnerabilidades de Dependabot

Revisadas vía API de GitHub: 20 altas, 16 moderadas, 1 baja. Confirmado que 6 de las 8 dependencias vulnerables (axios, form-data, multer, nodemailer, tmp, qs) ya estaban corregidas en esta rama por trabajo de sesiones anteriores. Actualizadas las 2 restantes: `sharp` 0.33.5→0.35.3 (dependencia central del pipeline OCR, probada a fondo: suite completa + build Docker real + operación de imagen real en el contenedor) y `fast-uri` 3.1.0→3.1.4. `npm audit fix` automático se probó y se descartó (empeoraba 2→9 vulnerabilidades por reordenar transitorias) — revertido. Quedan `body-parser` (baja) y `brace-expansion` (alta) sin tocar, transitorias, fuera de las 37 alertas originales. Los PRs individuales de Dependabot quedarán redundantes al fusionar esta rama. Desplegado (rebuild + stop + up -d backend), 4/4 healthy.

### 2026-07-29 — Cierre de la verificación ground truth: replay, hallazgo real de duplicado no detectado y 4 features permanentes en el panel admin

Verificación humana completada sobre las 29 facturas reales (`eval/facturas/`): 6 errores de OCR confirmados (#2, #4, #5, #15, #21 ya corregido a mano por Julio, #22), 1 pareja de duplicados reales no detectados (#4/#15) y 1 falso positivo descartado (#16/#24 — mismo proveedor, pedidos distintos).

- **Replay ejecutado** (`eval/replay.js`) contra las 29 facturas con el pipeline v2 completo (variantes CLAHE + Tesseract + aprendizaje ya activos): 94,4% de precisión (134/142 campos comparables, excluyendo ilegibles/ausentes) contra la verdad verificada. El propio script falló al escribir `docs/ocr-v2/INFORME-REPLAY.md` (`/app/docs` no está montado en el contenedor, gap ya documentado pero no corregido) — datos ya persistidos en `extracciones_v2` (modo='replay'), informe reconstruido a mano desde esas filas con `build_informe_replay.py`. Hallazgo relevante: v2 introduce una alucinación de fecha en la #22 (año 2023 en vez de 2026, error que v1 NO tenía) y falla sistemáticamente en extraer `numero_factura` con formato `26#XXXX` (#5, #16, #18, #19).
- **Causa raíz del duplicado no detectado (#4/#15)**: el índice de duplicados compara `proveedor_nif`, y el CIF de Coca-Cola en esa factura es realmente ilegible — cada subida obtuvo una lectura OCR distinta del NIF (`A28017895` vs `A08001851`), así que nunca coincidieron pese a tener idéntico `numero_factura`, `fecha_emision` y `total_factura`.
- `src/lib/duplicate-detector.js` (nuevo, testado): agrupa por `(user_id, fecha_emision, total normalizado, numero_factura normalizado)`, ignorando el NIF. Calibrado contra los 4 casos reales conocidos (positivo #3/#14, positivo #4/#15 con NIF distinto, negativo #5, negativo #16/#24).
- `server.js`: segunda señal de bloqueo en `/api/upload-confirm` (además de la existente NIF+fecha+total) que rechaza la subida si coincide número+fecha+total con una factura ya existente, sin importar el NIF. `GET /api/admin/facturas` ahora calcula `posible_duplicado`/`duplicado_grupo` sobre toda la tabla activa, no solo la página filtrada.
- Nueva columna aditiva `uploads.campos_no_fiables` (JSONB, default `[]`) — con rollback (`scripts/rollback/2026-07-29-uploads-campos-no-fiables-down.sql`).
- `PUT /api/admin/facturas/:id`: valida `invoice_type` (solo `compra`/`venta`) y acepta `campos_no_fiables` (whitelist de 10 campos "hoja").
- **4 features permanentes nuevas en el panel admin** (`admin-facturas.js/.html/.css`), sustituyen/complementan la herramienta temporal de ground truth:
  1. Click en la columna TIPO alterna Recibida/Emitida con confirmación — antes solo se podía corregir a mano en BD.
  2. Banner de aviso + filas en rojo pálido + botón "🗑️ Duplicado" (solo visible si aplica) que abre un modal comparando las facturas del grupo y deja elegir cuál conservar.
  3. Botón 🚩 junto al lápiz de edición en los 10 campos "hoja" (NIFs, números, fechas, importes) para marcar un dato como no fiable — la celda queda en rojo hasta desmarcarla.
- Corrección puntual de datos: `uploads.id=2` cambiado de `invoice_type='compra'` a `'venta'` (factura emitida por LUMAPA2 BROKERS SL, no recibida) — no requirió tocar `proveedor_nombre`/`receptor_nombre` porque `computeDisplayCompanies()` ya resolvía el display correctamente vía coincidencia de CIF; auditado en `audit_logs`. También `uploads.id=22`: `numero_factura` corregido de `0366/23/00246261` a `0306/23/00246261` (dígito `6`→`0` mal leído por v1), auditado igual.
- Pendiente de decisión de Julio: cuál de #4/#15 conservar (se resuelve desde el nuevo botón del panel, no unilateralmente desde la sesión).
- **Investigado y corregido el fallo sistemático de v2 en `numero_factura` formato `26#XXXX`** (hallazgo del replay, #5/#16/#18/#19): diagnóstico con llamadas reales aisladas (sin pasar por el replay completo) confirmó que Azure DI (`prebuilt-invoice`) devuelve `null` en `numero_factura` para estos documentos — no reconoce el campo, no es un problema de formato/comparación de texto — mientras Gemini Flash sí lo lee bien en 3 de los 4 casos (#18 falla en ambos motores, caso genuinamente ilegible). `resolverCampoSimple()` en `src/pipeline/arbiter.js` trataba "un motor no encontró nada" igual que "los motores discrepan", mandándolo a arbitraje y perdiendo el único valor disponible. Corregido: si un motor devuelve `null` y el otro un valor concreto, se acepta ese valor sin disputa (mismo principio que ya aplicaba `resolverIdentificador` a los NIF vía checksum). Verificado con los valores reales de los 4 casos: resuelve #5/#16/#19 correctamente, #18 queda en `null` como corresponde (ningún motor lo encontró). 3 tests nuevos en `pipeline-arbiter.test.js`.
- Suite completa: 302 tests, 301 OK (mismo fallo preexistente de paridad v3, no relacionado). Cache-buster `admin-facturas.{js,css}` → `20260729-001`.

### 2026-07-29 — Palancas de latencia OCR v2: 22,5s → 8,8s (−61%)
- Aprobado por Julio. Aplicadas y desplegadas 3 palancas: Tesseract en paralelo con la extracción (código en `orchestrator.js`, sus ~5,5s se solapan), `ocr_extraccion_v2_variantes_enabled=false` (evita 2ª pasada ~7,3s), y nuevo flag `ocr_extraccion_v2_arbitro_bloqueante=false` (código) que saca el árbitro OpenAI (~8,9s) del camino crítico → disputas a revisión humana.
- Re-medición en producción sobre 3 facturas: núcleo del pipeline v2 de 22.566ms a **8.757ms**, por debajo del objetivo de 12s → un preview síncrono de v2 es viable en backend (falta medir la subida móvil E1-E2 en dispositivo real).
- Suite 328/329 (fallo = paridad v3 preexistente). Deploy con rollback etiquetado `setex-prod-backend-rollback:2026-07-29-prelatencia`, 4/4 healthy, HTTPS 200. `features.json` commiteado (repo sincronizado con la config real).
- Tradeoff del árbitro no bloqueante: en sombra las disputas que OpenAI resolvería van a `pendiente_revision`; reversible por flag. Árbitro asíncrono real = Fase 1 del preview. Siguiente cuello: Mistral (7,3s) vs Gemini (5,5s).

### 2026-07-29 — Despliegue y activación en sombra de la pila OCR v2 nueva (sin Azure) + medición de latencia
- Aprobado por Julio. Desplegada la rama `feature/ocr-v2-modelos-configurables-2026-07-29` a producción: imagen anterior etiquetada `setex-prod-backend-rollback:2026-07-29-premodelos`, build+stop+up-d, 4/4 healthy, HTTPS 200, sin errores. Solo backend.
- Activados en `features.json` (caliente): `ocr_extraccion_v2_modelos_base:["gemini_flash","mistral"]` + `ocr_extraccion_v2_modelo_arbitro:"openai"`. Se disparó el gotcha del bind-mount (contenedor veía 0) → resuelto con stop+up-d. Verificado: `resolverConfigModelos` en el contenedor da base gemini+mistral, árbitro openai, ruta multi.
- Verificación end-to-end (coste ~$0.03 autorizado): `ejecutarPipelineV2Sombra` sobre factura real → confianzas gemini_flash 0.92 + mistral 0.93, árbitro OpenAI por disputa, sin escribir en BD (pool=null). La pila nueva corre en producción sin Azure. El usuario sigue viendo v1 (integración en preview aún no hecha).
- Medición de latencia (pila nueva, `docker exec node` sobre 3 facturas): gemini_flash 5,5s · mistral 7,3s · openai árbitro 8,9s · tesseract 5,5s. Núcleo 22,5s = apilamiento SERIE base(7,3)+árbitro(8,9)+tesseract(5,5). No hay una etapa dominante única; ninguna es Azure. Palancas cuantificadas en `PLAN-ESTUDIO-LATENCIA-CAPTURA.md`: Tesseract en paralelo (-5,5s), variantes off (-7,3s), árbitro fuera del camino síncrono. Preview síncrono <12s viable solo con esos 3 cambios.
- Rollback disponible: quitar los 2 flags (vuelve a legacy azure+gemini) o imagen de rollback etiquetada.

### 2026-07-29 — OCR v2: selección configurable de modelos (sin Azure) + plan de estudio de latencia
- Decisión de Julio: retirar Azure del pipeline v2 de momento (dejándolo listo para cuando se contrate S0) y tirar con Gemini Flash + Mistral OCR 4 + OpenAI 4.1, con un selector para incluir/quitar 2-4 modelos base + árbitro opcional en caliente. Techo de coste €500/mes para el pico de transición. Dataset: las 28 facturas reales existentes.
- Rama `feature/ocr-v2-modelos-configurables-2026-07-29` (desde HEAD del runtime, no main), commit `feat(ocr-v2): selección configurable de 2-4 motores base + árbitro opcional`. Sin desplegar.
- Ficheros (todos v2, ningún fichero v1 tocado): `pipeline/seleccion-modelos.js` (nuevo), `pipeline/extractors.js` (+dispatch por nombre + `ejecutarExtraccionV2Multi`), `pipeline/arbiter.js` (+`arbitrarFacturaMulti`), `pipeline/orchestrator.js` (generalizado con helper `extraerYArbitrar`).
- Diseño de seguridad: sin flags nuevos, comportamiento idéntico a hoy (ruta legacy azure+gemini que los 20 tests del orquestador siguen ejercitando); la mezcla de modelos se cambia por `features.json` (bind-mount, no tocado aún). Árbitro N-modelos por torneo que reutiliza el pairwise probado (con 2 modelos es idéntico). Consecuencia: sin Azure no hay re-extracción dirigida (solo Azure da bounding boxes).
- Tests: 56 nuevos, suite 327/328 verde (el único fallo es la paridad v3 preexistente). Config recomendada a activar al desplegar documentada en `CONFIG_RECOMENDADA`.
- Nuevo `docs/plans/PLAN-ESTUDIO-LATENCIA-CAPTURA.md`: marco exhaustivo del estudio foto→resultado (12 etapas E1-E12), hipótesis fundamentadas, instrumentación (script de laboratorio + medición en dispositivo real para la red móvil), y entregable. Hallazgo: `extracciones_v2` no persiste tiempos por etapa; los 30 s medios de sombra están contaminados por reintentos 429 de Azure F0 + variante x2.
- Pendiente (gates de Julio): desplegar la rama, añadir flags a features.json, sesión de dispositivo para latencia, reactivar Azure cuando se contrate S0.

### 2026-07-29 — Verificación de realidad para activar OCR v2: tres muros duros medidos
- Investigación de solo lectura sobre BD y sombra antes de ejecutar el plan. Registro completo en `docs/plans/PLAN-ACTIVACION-OCR-V2.md` §H.
- **Muro 1 — datos**: `uploads` tiene **28 facturas en toda la historia**; el plan exige ≥100 verificadas para decidir y ≥500 de sombra para validar. No alcanzable hoy: no existen esas facturas. El producto aún no tiene tráfico real (el volumen 5.000-6.000/mes es el esperado del cliente, no el realizado).
- **Muro 2 — Azure**: solo Julio puede ver/contratar el tier en el portal; con 28 facturas de vida el comportamiento no revela el tier. Tarifa S0 verificada: $10/1.000 páginas.
- **Muro 3 — latencia**: v2 en sombra mide **30 s de media, 74 s máx** (parte por reintentos 429 de F0). El preview v1 tarda 2-5 s. Si S0 no baja la latencia de v2 bajo 12 s, el diseño síncrono de la Fase 1 no es viable y v2 necesitaría UX asíncrona. Medir en S0 es precondición del diseño.
- **Coste real medido** (`extracciones_v2.coste_estimado_usd`, 56 filas): $0.00825/factura SIN Azure (F0 gratis) → con S0 ≈ $0.028/factura con `variantes`. Proyección: 6.000/mes ~€155, pico 15.000/mes ~€390 (al límite del techo €400 de Julio); la transición sombra v1+v2 a pico se pasa de €400 → palanca: apagar `variantes`.
- **Calidad** (`eval/comparar-v1-v2.js` re-ejecutado): v2 96.3% vs v1 90.1% en campos de documento; 9 de las 10 "regresiones" de v2 son el artefacto de identidad ya conocido, no OCR. Confirma que v2 lee mejor el papel.
- Conclusión: activar hoy sería decidir sobre 28 muestras — el error de fondo de LL-002. Acción que desbloquea: Azure S0 (Julio) → medir latencia real → decidir diseño Fase 1.

### 2026-07-29 — Plan de activación OCR v2 reescrito a versión 2.0 ultradetallada
- `docs/plans/PLAN-ACTIVACION-OCR-V2.md`: reescritura completa (v1.0→v2.0) para blindar la ejecución por sesiones IA futuras — contrato de ejecución §A con reglas vinculantes y anclas de código verificadas contra el fuente real.
- Correcciones sobre v1.0: punto de enganche real es `server.js:1819` (no 1800), herramientas eval en `app/backend/eval/` (no `eval/`), bloque sombra confirm en `server.js:2916-2945`.
- Riesgos nuevos R9-R11: doble coste v2 en sombra (preview+confirm, mitigado vía Redis en Fase 1.5), deploy desde `main` descalzado (REGLA 11) y deriva de interpretación en sesiones IA.
- Hallazgo de coste: los ~$0.008/factura del replay se midieron con Azure F0 (gratis) — la cifra NO incluye Azure; recalcular en Fase 0.2 con tarifa S0 real.
- Diseño de despliegue afinado: flag único `ocr_extraccion_v2_modo` con 4 valores (off/sombra/canario/activo), canario por lista `ocr_extraccion_v2_pilotos`, activación por porcentaje con hash determinista de userId, fallback a v1 con timeout 10 s.
- Añadidas FASE 6 (consolidación de v2 como motor definitivo tras ≥3 meses con métricas, vía ADR nuevo) y bitácora de ejecución §H obligatoria.

### 2026-07-29 — Vista OCR: tres columnas honestas, endurecimiento de known_cifs y formato único de importes

Investigación a fondo de una paradoja reportada por Julio: en el modal "Vista OCR" había campos donde arriba se indicaba que la IA había coincidido con el humano, pero abajo, en el ranking por motor, **ningún motor tenía ese valor**.

- **Causa raíz**: la columna "IA (OCR raw)" no era raw. Mostraba `ocr_result.merged`, que es el resultado del sistema **después** de fusionar motores, recalcular aritmética y sobrescribir campos desde `known_cifs`, `company_catalog`, `company_relationships` y `users`. `const campos = ocrData.campos` (server.js) es un **alias por referencia**: todas las mutaciones del endpoint machacaban el resultado de la IA en el sitio, sin dejar rastro. Demostrado con datos reales: factura #31, la IA leyó `Roberto Ramírez Sánchez, S.L.` y el panel mostraba `ALEX DISTRIBUCIONES` (de `known_cifs`, 9 confirmaciones, `last_seen` idéntico al `procesado_en` de la factura). Barrido SQL: **42 pares campo-factura** con "merged == confirmado y ningún motor == confirmado" — es decir, ~20% de los "aciertos" del panel eran falsos y cualquier métrica sacada de ahí estaba inflada.
- **Segundo mecanismo, sin BD de por medio**: factura #22, `iva_porcentaje` = `21,0` en merged, `10,0` en Gemini, nada en Azure. Lo impuso `reconcileMultiIvaAggregates` (tipo del tramo de mayor cuota). El propio sistema lo etiquetaba `calculated` en `campo_sources`, pero el modal lo pintaba como lectura de IA.
- **Tres columnas honestas**: nuevas claves `ocr_result.ia_pura` (clon del merged **antes** de las sobrescrituras) y `ocr_result.overrides_bd` (campo, valor de la IA, valor aplicado, fuente). El modal pasa a *Leyó la IA · Decidió el sistema · Confirmado humano*, y el acierto se mide 1 vs 3. Solo para facturas nuevas: en las 28 anteriores ese dato nunca se guardó y no es recuperable (se muestra `n/d`, sin inventarlo).
- **Identidad del usuario**: se confirma como comportamiento deliberado que el nombre y CIF de la empresa que hace la foto se tomen **siempre** del registro y nunca de la IA. Lo que cambia es que ya no se disfraza de acierto: sale etiquetado `registro del usuario`.
- **`known_cifs` endurecido (riesgo de seguridad)**: se separa `knownProvider` (aplicar el valor conocido) de `knownProviderFiable` (saltarse la revisión humana). Antes bastaba **una** confirmación humana —posiblemente errónea— para sobrescribir la lectura de la IA *y además* empujar a auto-confirmación, convirtiendo el error en verdad permanente que nadie volvía a mirar. Ahora hacen falta ≥2 confirmaciones, o que venga del catálogo curado por un admin.
- **`campo_sources` deja de estar desfasado** sin recalcularlo: describe `ia_pura` (se computa en `ocr/index.js` justo tras la fusión), así que se etiqueta como tal (`campo_sources_describe`) y el frontend lo pinta junto a la columna correcta.
- **`total_factura` con coma en toda la app**: `normalizeTotal` devolvía punto (`toFixed(2)`) mientras el resto de importes usaban coma; la edición manual del admin guardaba lo que se tecleara. Migradas 18 filas (28/28 con coma, 0 colisiones en el índice de duplicados, auditado, con rollback). Nueva función SQL `setex_total_num()`. **Arreglada la suma de `/api/admin/users`**, cuya regex solo casaba el formato con punto: las facturas con coma sumaban 0 sin avisar de nada. Normalización extendida a `base_imponible`/`cuota_iva`/`cuota_irpf` en la edición admin para evitar que vuelva a desviarse.
- **Bugs colaterales corregidos**: `fmtOcrImporte` leía `1.234,56` como 1,23 € (parseFloat ingenuo); `normCmp` del panel no pasaba a mayúsculas ni comparaba números por valor (`21` vs `21,0` salía en rojo sin discrepancia real) — ahora alineada con `normalizarParaComparar` del backend; la columna Total llevaba `sorter:'number'`, que con coma ordenaba mal.
...
- `app/frontend/src/index.html`: contenedor `#progress-overlay` (role=progressbar + aria) tras `#upload-btn`; `progreso.js?v=20260821-001`; cache-buster app.js→`?v=20260821-001`.
- `app/frontend/src/styles.css`: bloque nuevo al final (overlay parcial pointer-events:none, barra, prefers-reduced-motion); sin tocar estilos existentes.
...
- **Backend** (`pipeline/`): `fusion-multipagina.js` (fusiona N canónicos en 1 factura reutilizando validadores `iva.js`/`nif.js`; cabecera de la 1ª hoja, líneas unidas, total coherente de la hoja de importes; devuelve `camposFaltantes` con zona fiscal/importes), `orquestador-multipagina.js` (extrae cada página con el pipeline v2 → fusión), `adaptador-v1.js` (canónico→plano, test de contrato). Endpoint separado `POST /api/upload-preview-multipagina` (NO toca `upload-preview` — lección LL-002), flags `ocr_multipagina_enabled` (off) + `ocr_multipagina_max_paginas` (6, bloqueante), magic bytes por página.
- **Frontend** (`multipagina.js` + `index.html` + `styles.css`): captura múltiple fotos-primero con miniaturas (añadir/reordenar/quitar), aviso dirigido cuando falta un dato, reutiliza `showConfirmModal`. PDF degrada con aviso si pdfjs no está cargado.
- 30 tests nuevos en total (fusión 9, orquestador 4, adaptador 4, + selección/multi previos). Suite 345/346 (fallo = paridad v3 preexistente). `node --check` OK. **Nada desplegado; flujo de una foto intacto.**
- Pendiente: persistencia en confirm (columna `paginas JSONB`), verificación en navegador real, y despliegue por staging con el flag.

### 2026-08-01 — Hiperdocumento de migración del frontend de usuario final

- `docs/HIPERDOC-FRONTEND-USUARIO-v1.html` (nuevo, autocontenido, 110 KB): especificación completa en 20 secciones del frontend de usuario final para trasladarlo a una aplicación nueva sin releer el código original. Motivo: petición de Julio de disponer de un documento descargable de transferencia.
- Alcance documentado: `index.html`, `app.js`, `styles.css`, `auth.js`, `cif-validator.js`, `service-worker.js`, `manifest.json` (3.507 líneas propias). El panel admin queda expresamente fuera.
- Contenido: inventario y arquitectura de carga; sistema de diseño extraído (paleta, escala, radios, chip determinista de empresa); máquina de estados de las 5 pantallas; contrato de `auth.js` (AT en memoria + RT cookie, `apiFetch`, BroadcastChannel); cámara y jscanify/OpenCV con su fix de carrera `cv.then()`; contratos exactos de `/api/upload-preview` y `/api/upload-confirm`; motor fiscal cliente (snap {21,10,4,0}, dedupe y tope de 4 tramos, tolerancias 0,02/0,05 €, corrección ×1000, resumen bidireccional); validación AEAT y emisor/receptor; historial de 15 columnas; PWA; microcopy literal; checklist de paridad.
- Deuda señalada para no replicar: HTML sin escapar en el historial (`renderHistoryTable`), `onclick` inline incompatible con CSP endurecida, `catch {}` silenciosos que ocultan fallos de `/api/me/settings` (deja la app sin CIF de empresa y anula la validación cruzada), 8,9 MB de OpenCV sin medición de beneficio, `pdf.min.js` cargado en disco pero no referenciado, y tres analizadores distintos de importes en formato español.
- Sin cambios en código ni configuración: la entrega es exclusivamente documental.

### 2026-08-01 — Vista OCR: "¿Acertó la IA?" pasa a medir 2 vs 3 (sistema vs confirmado)

- `app/frontend/src/admin-facturas.js` (~línea 2397): la columna de acierto del modal Vista OCR comparaba columna 1 (`ia_pura`, lectura pura de la IA) contra columna 3 (confirmado). Pasa a comparar **columna 2 (`ocr_result.merged`, lo que el sistema decidió y presentó al usuario en el modal de confirmación) contra columna 3**. Motivo (petición de Julio): la métrica relevante es qué le llega al cliente, no la precisión del motor OCR aislado.
- Efecto colateral positivo: desaparece el `n/d` de las facturas anteriores al 29/07/2026 — `merged` existe en todas, `ia_pura` solo desde esa fecha. La columna 1 sigue visible con sus badges de motor, y el ranking multi-motor sigue midiendo la precisión por IA.
- Coherencia interna: la fila "Tramos IVA" de esa misma tabla ya comparaba `raw` (col 2) vs `confirmed` (col 3). Ahora todas las filas usan el mismo criterio.
- Renombrada la columna a **"¿Acertó el sistema?"** y reescrita la leyenda. Mantener el rótulo "¿Acertó la IA?" sobre una comparación que incluye sobrescrituras de BD reintroduciría exactamente la lectura falsa corregida el 29/07/2026 (~20% de aciertos inflados).
- `app/frontend/src/admin-facturas.html`: cache-buster `?v=20260729-005` → `?v=20260801-001` (regla 6).
- **Pendiente de despliegue**: el frontend se copia en la imagen (sin volumen), así que requiere `docker compose build frontend && stop && up -d`. No ejecutado — a la espera de OK de Julio.
- Staging no se ha tocado: su `admin-facturas.js` diverge (no contiene la tabla de 3 columnas).

### 2026-08-01 — Ranking por motor: añadida la fila de Tramos IVA

- `app/frontend/src/admin-facturas.js` (~línea 2515): la tabla "🏆 Ranking por motor" no mostraba los tramos de IVA porque se genera con `CAMPOS.map()`, que solo contiene los 12 campos escalares; en la tabla principal los tramos se pintan como fila manual (`rowLineas`) y esa adición nunca se replicó en el ranking. No era falta de datos: `motors.<engine>.campos.lineas_iva` ya viajaba en `/ocr-detail` y lo consumía `tramoBadge()` para la atribución por tramo de la columna 1.
- Nueva fila "Tramos IVA" añadida fuera del bucle de CAMPOS. Como no hay valor escalar que comparar, cuenta **cuántos de los tramos confirmados reportó cada motor** (`tramoMatches`: tipo de IVA exacto + importes con tolerancia del 2%), con marcador `n/total`: verde = todos, ámbar = algunos, rojo = ninguno, `—` = el motor no devolvió tramos. Sufijo `(+N no confirmado)` cuando el motor devuelve más tramos de los confirmados.
- Verificado con datos reales de la factura #30 antes de desplegar: OpenAI `2/2`, Azure `2/2 (+1 no confirmado)` — Azure añade un tramo espurio `{base:null, cuota:"4,41", porcentaje:null}` que el marcador ahora hace visible. Ese sufijo detecta alucinación de tramos, que antes no se veía en ningún panel.
- `app/frontend/src/admin-facturas.html`: cache-buster `?v=20260801-001` → `?v=20260801-002` (regla 6).
- Desplegado: `docker compose build frontend && stop && up -d`. Contenedor healthy, HTTPS 200. Backend, postgres y redis intactos.

### 2026-08-01 — Hiperdocumento del frontend: solo Markdown, retirado el HTML

- `docs/HIPERDOC-FRONTEND-USUARIO-v1.md` (nuevo, 64 KB, 1.043 líneas): misma especificación de 20 secciones reescrita como Markdown nativo, no convertida mecánicamente (pandoc habría arrastrado los `style` inline y roto las 24 tablas). Añade índice con anclas y convierte el checklist de paridad en casillas `- [ ]` marcables. Versionable en git y comentable en PR, cosa imposible con el HTML.
- `docs/HIPERDOC-FRONTEND-USUARIO-v1.html`: **borrado** a petición de Julio, tras verificar que el `.md` contiene la misma información (20 secciones, 24 tablas, 18 bloques de código).
- Corregidos los avisos de markdownlint del fichero nuevo: MD060 (separadores de tabla normalizados a `| --- |`, respetando las alineaciones `---:`) y MD036 (subtítulo de portada en negrita → texto plano). Verificado con diff que solo cambiaron las líneas separadoras y la línea 3.
- El artefacto publicado en claude.ai sigue vivo con el contenido HTML original; es independiente del fichero borrado.

### 2026-08-10 — Botón "ojo" para mostrar/ocultar la contraseña en todos los formularios

- Petición de Julio: poder ver la contraseña mientras se escribe, en el login y para cualquier usuario. Implementado sobre los **6 campos** `input[type=password]` del producto: login de usuario, registro y las dos casillas de restablecer contraseña (`index.html`), login de admin (`admin-login.html`) y login embebido del panel (`admin-facturas.html`).
- `app/frontend/src/auth.js` (+100 líneas al final): función `initPasswordToggles()` que recorre todos los `input[type=password]`, los envuelve en `.pwd-wrap` e inyecta un `<button class="pwd-toggle">` con SVG inline (iconos Feather eye/eye-off). Vive en `auth.js` y no en `app.js`/`admin-login.js`/`admin-facturas.js` porque es el **único script cargado por las tres páginas** con campos de contraseña: una sola implementación en vez de tres copias divergentes. Es idempotente (`data-pwd-toggle`) y no toca formularios ni handlers existentes.
- `btn.type = 'button'` explícito: dentro de un `<form>` el valor por defecto es `submit` y pulsar el ojo habría enviado el login en lugar de revelar el texto. `tabIndex = -1` para que el tabulador salte del campo al botón "Entrar". `aria-label` + `aria-pressed` + `title` conmutan con el estado.
- Seguridad: la contraseña **vuelve a ocultarse sola** en dos situaciones — al enviar cualquier formulario (listener `submit` en fase de captura, para correr aunque otro handler haga `preventDefault()`) y cuando el campo deja de ser visible (`IntersectionObserver`, cubre el cambio de pantalla de la PWA, que no usa `<form>`). Evita dejar la contraseña en claro tras entrar (shoulder surfing en un móvil sobre la mesa).
- `app/frontend/src/styles.css` y `admin-facturas.css`: estilos de `.pwd-wrap`/`.pwd-toggle`. En `styles.css` hay que neutralizar explícitamente las reglas globales `input`/`button` del propio fichero (`width:100%`, `margin`, y sobre todo el `transform: translateY(-1px)` de `button:hover`, que sacaba el ojo fuera del campo al pasar el ratón): se reafirma `translateY(-50%)` con un selector de mayor especificidad.
- Cache-busters (regla 6): `styles.css` y `admin-facturas.css` → `?v=20260810-001`; `auth.js` → `?v=20260810-001` en los tres HTML.
- Desplegado en prod por build manual + swap (`docker compose build frontend && stop && up -d`), **no** por `deploy-prod.yml` (regla 11). Contenedor healthy. Rollback disponible: `setex-prod-frontend:rollback-20260810` (`487995f4b528`).
- Verificación funcional: el sitio estaba dentro de su ventana de cierre nocturno (404 de `@bloqueado` vía `auth_request /api/internal/check-access`, comportamiento correcto y ajeno al cambio), así que se validó levantando la **misma imagen** en un contenedor temporal en `127.0.0.1:8099` con un nginx mínimo. Comprobado con Playwright: los 6 campos equipados, alternancia `password`↔`text`, foco devuelto al input con el cursor al final, el clic no envía el formulario, el ojo queda dentro del campo y centrado verticalmente, y el `submit` restaura `type=password`. Contenedor temporal eliminado.

### 2026-08-11 — Borrado total de facturas: rastro OCR completo + modal de confirmación

- Petición de Julio: borrar una factura entera desde cualquier admin de la asesoría (`role='admin'` y `role='tech'`), eliminado total de BD, con confirmación en ventana emergente. Auditoría previa: el borrado **ya existía** (`DELETE /api/admin/facturas/:id`, botón 🗑 Eliminar del panel, accesible a ambos roles vía `requireAdmin`); lo que faltaba era que el borrado fuese realmente total y que la confirmación dejase ver qué factura se borra.
- **`app/backend/src/server.js`** (handler DELETE reescrito): el borrado anterior solo eliminaba la fila de `uploads`. `extracciones_v2` y `ocr_benchmark_resultados` sí caían por `ON DELETE CASCADE`, pero **`ocr_shadow_comparativa`, `ocr_imagen_variante_comparativa` y `ocr_shadow_validaciones` se enlazan por `preview_id` y no tienen FK**: sobrevivían con datos fiscales del cliente (proveedor, NIF, total). Ahora se purgan explícitamente por `preview_id`, acotado a `preview_id` no nulo (un NULL casaría con filas de otras facturas y borraría de más).
- Se borra también **`ruta_variante`**, la imagen de contraste del bloque 5, que quedaba en disco: era una foto legible de la factura que sobrevivía a un borrado "total". Se lee antes de la transacción porque su fila desaparece dentro de ella.
- Todo el borrado en BD va en **una transacción** (`BEGIN`/`COMMIT`/`ROLLBACK` + `client.release()` en `finally`), siguiendo el patrón de `DELETE /api/me/account`. Los ficheros se borran **después** del COMMIT: al revés, una transacción fallida dejaría una fila viva apuntando a una imagen ya destruida (irrecuperable), mientras que el orden elegido solo puede dejar una fila sin fichero (recuperable).
- **Seguridad**: `unlink()` ya no se ejecuta sobre una ruta sin validar. Ahora `path.resolve()` + comprobación `startsWith('/app/uploads/')` para los dos ficheros, igual que ya hacía `/imagen-variante` (`server.js:3299`). Un `unlink` sin validar es una primitiva de borrado arbitrario si alguna vez se logra escribir en esas columnas. `ENOENT` se distingue de un fallo real. La auditoría `ADMIN_DELETE_FACTURA` incluye ahora recuento por tabla y desenlace de cada fichero.
- `failed_jobs.upload_id` se mantiene `ON DELETE SET NULL`: la traza del fallo debe sobrevivir al borrado del documento.
- **`app/frontend/src/admin-facturas.{html,js,css}`**: la confirmación pasa de `confirm()` nativo (solo mostraba el número) a un modal propio del panel con proveedor, NIF, fecha e importe, la lista de lo que se destruye y el aviso de irreversibilidad. Botón destructivo rojo (`.btn-peligro`) distinto del primario azul. Cancela con backdrop y con Escape. Los errores se muestran **dentro** del modal (un `alert()` lo cerraría y obligaría a reseleccionar la fila). Si el modal no existe en el DOM (HTML cacheado antiguo) la función no borra a ciegas: no hace nada.
- Se mantiene el modo eliminar de dos pasos (🗑 Eliminar → ✕ por fila), decisión de Julio: fricción deliberada frente al borrado accidental de datos fiscales.
- **Huérfanos históricos: NO se han limpiado, decisión revertida sobre el terreno.** Se autorizó limpiar las 24 filas huérfanas, pero la comprobación previa demostró que las 8 de cada tabla corresponden a **previews nunca confirmados**, no a residuo de borrados (`upload_id IS NULL` en las 8 de `ocr_shadow_comparativa`, y los mismos `preview_id` en las otras dos). Son los datos que el panel shadow muestra a propósito como "⚠ Sin confirmar": borrarlos habría destruido información válida del análisis del pipeline v2.
- Cache-busters (regla 6): `admin-facturas.css` y `admin-facturas.js` → `?v=20260811-001`.
- Desplegado por build manual + swap de **backend y frontend**, no por `deploy-prod.yml` (regla 11). Backup previo de BD verificado (`setex_db_20260811_170050.sql.gz.gpg`). Los 4 contenedores healthy, portal y admin 200. Rollback: `setex-prod-backend:rollback-20260811` y `setex-prod-frontend:rollback-20260811`.
- **Verificación parcial, pendiente de prueba end-to-end**: validados sintaxis (`node --check`), arranque sano, y el modal renderizado con el CSS real (Playwright sobre la imagen desplegada en un contenedor temporal). **El borrado real no se ha ejecutado ni una sola vez**: requería insertar una factura sintética en producción y la salvaguarda del entorno lo bloqueó. Queda pendiente que Julio borre una factura de prueba y se verifique que no queda rastro en las 3 tablas ni en disco.

### 2026-08-12 — La columna "Acciones" era inalcanzable: bloque congelado a la derecha por una columna oculta

- Julio reportó que no encontraba el borrado en el panel: la columna **"Acciones" quedaba tapada** por lo que describió como una segunda columna ID fija a la derecha. Diagnóstico: la tabla de facturas declaraba **dos** columnas con `frozen: true` — `ID` (la legítima, a la izquierda) y **`Código cliente`**, oculta desde el 2026-07-29 con `visible: false` pero conservando `frozen: true`. Tabulator sigue montando el contenedor de columnas congeladas aunque la única que lo ocupa esté oculta; ese bloque se ancla a la **derecha** y se superpone a la última columna, que es precisamente "Acciones".
- **`app/frontend/src/admin-facturas.js`**: eliminada la definición de la columna `Código cliente` de la tabla de facturas. Verificado antes de borrarla que las otras 6 referencias a `codigo_cliente` del fichero (líneas ~864, ~899, ~992, ~1203, ~1326, ~1341) pertenecen todas a la tabla de **empresas** (`client_companies`) y a su editor, no a la de facturas: el borrado no las afecta. Queda una sola columna `frozen: true`, la de ID a la izquierda, sin tocar (petición literal de Julio: "solo quiero la de la izquierda, tal cual").
- **`persistenceID` subido a `setex-admin-facturas-v11`**: Tabulator persiste el orden de columnas en localStorage bajo esa clave. Sin invalidarla, un layout guardado que aún listara `Código cliente` la reintroduciría al final del orden y volvería a congelar un bloque a la derecha sobre "Acciones" — exactamente el fallo que documenta el comentario de v10 el 2026-07-29. Es el motivo de que el bug pudiera persistir en el navegador de Julio aunque el código fuese correcto.
- Cache-buster (regla 6): `admin-facturas.js` → `?v=20260812-001`.
- **Rollback etiquetado ANTES del build** (`setex-prod-frontend:rollback-20260812`, `6cc75d42f5a5`), corrigiendo el defecto detectado el día anterior: `setex-prod-backend:rollback-20260811` apuntaba a una imagen del 29-jul y no revertía al estado previo real. La imagen de rollback debe etiquetarse desde la que está en servicio, no reutilizar una antigua.
- Desplegado por build manual + swap solo frontend (`build` → `stop` → `up -d`), no por `deploy-prod.yml` (regla 11). Contenedor healthy. Verificado en la imagen servida: `setex-admin-facturas-v11` presente, `?v=20260812-001` en el HTML y **una sola** ocurrencia de `frozen: true`.
- Auditoría del borrado total durante el diagnóstico: el backend vivo sí purga las tres tablas sin FK (`ocr_shadow_comparativa`, `ocr_imagen_variante_comparativa`, `ocr_shadow_validaciones`). El último `ADMIN_DELETE_FACTURA` de `audit_logs` es del **2026-07-29** (facturas #14 y #15, con el código antiguo), confirmando que **el borrado total sigue sin estrenarse**. Las 24 filas huérfanas siguen siendo las de previews nunca confirmados, no residuo de borrados.
- Confirmado en el esquema que **`preview_id` no es clave foránea en ninguna tabla** (`ocr_shadow_comparativa`, `ocr_imagen_variante_comparativa`, `ocr_shadow_validaciones` → regla de borrado NINGUNA). Deuda abierta: cualquier tabla nueva del pipeline v2 reintroducirá la fuga en silencio.

### 2026-08-12 — El borrado total nunca pudo funcionar: `DELETE` contra una VISTA (error 55000)

- Al estrenar el borrado, Julio recibió `No se pudo eliminar la factura: Error al eliminar la factura`. Log del backend: `cannot delete from view "ocr_shadow_comparativa"` (SQLSTATE **55000**, *Views that do not select from a single table or view are not automatically updatable*).
- **Causa**: `ocr_shadow_comparativa` **no es una tabla, es una VISTA** creada en `server.js:441` (`ocr_shadow_validaciones s LEFT JOIN uploads u ON u.preview_id = s.preview_id`). Postgres rechaza `DELETE` sobre una vista con join. La sentencia abortaba la transacción entera, que hacía `ROLLBACK`: **el borrado fallaba siempre con HTTP 500 desde el 2026-08-11**. La funcionalidad nunca llegó a funcionar ni una vez.
- **Error de diagnóstico del 2026-08-11**: la auditoría que produjo el fix consultó `information_schema.columns`, que lista columnas de tablas **y de vistas** sin distinguirlas. La vista se contabilizó como tabla y se le añadió un `DELETE`. La comprobación correcta (`pg_class.relkind`) es una sola consulta y no se ejecutó. Resultado del recuento correcto: `ocr_imagen_variante_comparativa` y `ocr_shadow_validaciones` son TABLAS; `ocr_shadow_comparativa` es VISTA.
- **Corolario**: los "24 huérfanos" reportados el 2026-08-11 eran **16**. Las 8 de `ocr_shadow_comparativa` son las mismas 8 de `ocr_shadow_validaciones` contadas por segunda vez a través de la vista. La conclusión de fondo no cambia (son previews nunca confirmados, no se tocan).
- **`app/backend/src/server.js`**: eliminada la sentencia contra la vista. Es innecesaria además de errónea — la vista no tiene datos propios y se vacía sola al purgar `ocr_shadow_validaciones` y borrar la fila de `uploads`. Actualizados el contador `purgadas` (`{variante, validaciones}`), la línea de log y los comentarios de cabecera que describían la vista como tabla.
- **El diseño transaccional del 2026-08-11 hizo su trabajo**: el intento fallido no dejó daño. `ROLLBACK` completo, 34 facturas intactas, ninguna entrada de auditoría, ningún fichero borrado. Un borrado no transaccional habría destruido el rastro OCR y dejado la factura viva.
- **Verificado contra la BD real** antes de desplegar, en transacción revertida: `DELETE 1` en `ocr_imagen_variante_comparativa`, `DELETE 1` en `ocr_shadow_validaciones`, `DELETE 1` en `uploads`, sin error → `ROLLBACK`.
- Desplegado backend por build manual + swap (regla 11), rollback etiquetado **antes** del build (`setex-prod-backend:rollback-20260812`, `e45cb035e7eb`). Healthy, cero errores desde el arranque, confirmado que la sentencia contra la vista ya no existe en el código servido. Backup de BD del día disponible (`setex_db_20260812_030002.sql.gz.gpg`).

### 2026-08-12 — Retirado `frozen` de la columna ID: reaparecía anclada a la derecha sobre "Acciones"

- Tras eliminar `Código cliente`, Julio reportó que la columna ID **volvía a aparecer a la derecha** tapando "Acciones". Descartado por inspección: solo existe **una** columna titulada ID y es la primera; no hay columnas añadidas dinámicamente (`addColumn`/`setColumns` no aparecen); no hay reglas CSS de columna fija (el único `position: sticky` del fichero es el del header de página). Única causa posible restante: `frozen: true`.
- Tabulator reancla el bloque de columnas congeladas en cada `redraw(true)`, y el toggle del modo eliminar llama exactamente a `table.redraw(true)` (`admin-facturas.js:1726`) — de ahí que reapareciera justo al ir a borrar.
- **Decisión: retirar `frozen: true` de la columna ID** en vez de seguir ajustándolo a distancia (dos intentos fallidos previos). La columna sigue siendo la primera por la izquierda, que es lo que se quiere ver; solo deja de permanecer visible al hacer scroll horizontal. Contrapartida aceptada a cambio de que "Acciones" sea alcanzable. **La tabla de facturas ya no tiene ninguna columna congelada.**
- `persistenceID` → `setex-admin-facturas-v12` (un layout guardado con la columna congelada la seguiría anclando). Cache-buster: `admin-facturas.js` → `?v=20260812-002`.
- Desplegado frontend por build manual + swap. Healthy. Rollback: `setex-prod-frontend:prev-20260812`.
- Se intentó crear una factura sintética en producción para probar el borrado sin tocar datos de cliente, y **la salvaguarda del entorno bloqueó la escritura en la BD de producción**, igual que el 2026-08-11. La verificación la ejecutó Julio directamente desde el panel (ver entrada siguiente).

### 2026-08-12 — VERIFICADO end-to-end: el borrado total funciona y no deja rastro

- Julio borró la factura **#37** desde el panel a las 09:09:35 UTC. **Primer borrado real ejecutado con éxito** desde que la funcionalidad se entregó el 2026-08-11.
- Auditoría `ADMIN_DELETE_FACTURA`: `purgadas = {variante: 1, validaciones: 1}`, `ficheros = {original: "borrado", variante: "inexistente"}`. La factura tenía fila en `ocr_imagen_variante_comparativa` pero sin imagen de contraste en disco — el `ENOENT` se distinguió correctamente de un fallo real, tal y como se diseñó.
- **Rastro cero confirmado** para `upload_id=37` / `preview_id=91f73aa3-991f-4667-8a91-7baa60ae5cdc` en: `uploads`, `extracciones_v2`, `ocr_benchmark_resultados`, `ocr_imagen_variante_comparativa`, `ocr_shadow_validaciones`, la vista `ocr_shadow_comparativa` y `failed_jobs`. Fichero `…_1a1187.jpg` ausente del disco (`find /app/uploads` sin resultados).
- **Barrido ciego de toda la BD** (`scratchpad/barrido_rastro.sql`): en vez de comprobar una lista de tablas conocidas —el método que causó el fallo del 11-ago— se recorrieron **todas** las columnas `text`/`varchar`/`uuid`/`json`/`jsonb` de **todas** las tablas base de `public` (filtrando `relkind='r'`, sin vistas) buscando el `preview_id`, el número de factura y el nombre del fichero.
- Resultado del barrido: 18 coincidencias, **todas explicadas y ninguna de la factura borrada**:
  - `audit_logs.details` — el snapshot del propio borrado. Intencional (traza de auditoría).
  - `extracciones_v2` (`upload_id=36`), `ocr_benchmark_resultados` (`upload_id=36`, 9 filas), `ocr_imagen_variante_comparativa` (`preview_id=4d9fbfc2…`), `uploads.numero_factura`/`ocr_result` — **todas de la factura #36**, que es la MISMA factura del proveedor subida 35 segundos antes (`17:43:37` vs `17:44:12`, ficheros `…436646.jpg` y `…1a1187.jpg`). Es un duplicado real del usuario, con su propio `preview_id` y su propio fichero; sigue viva y debe seguir viva. Coincide por `numero_factura`, no por pertenecer a la borrada.
- **Observación RGPD (no resuelta, requiere decisión)**: el snapshot de `audit_logs` conserva datos fiscales completos del cliente (proveedor, NIF, receptor, importe, nº de factura) tras el borrado. Es correcto como traza de auditoría de una acción destructiva, pero **un borrado ejercido bajo el art. 17 RGPD (derecho de supresión) no quedaría satisfecho**: el dato sobrevive en la auditoría. Opciones a valorar: seudonimizar el snapshot, reducirlo a identificadores, o fijarle retención propia.

### 2026-08-21 — Eliminar empresa completa (Opción A) + fix visibilidad modo eliminar (DESPLEGADO en prod)
- `server.js` + `server.legacy.js`: `DELETE /api/admin/client-companies/:id` acepta `{confirmation: CIF}`; con usuarios borra en transacción facturas+usuarios+empresa (ficheros best-effort fuera de tx, known_cifs anonimizado, audit_logs SET NULL por FK); sin usuarios comportamiento original. Mensaje 409 sustituido («Desactívala primero» era imposible de cumplir: no había forma de borrar usuarios).
