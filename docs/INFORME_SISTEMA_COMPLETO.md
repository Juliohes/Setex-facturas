...
- Corrección puntual de datos: `uploads.id=2` cambiado de `invoice_type='compra'` a `'venta'` (factura emitida por LUMAPA2 BROKERS SL, no recibida) — no requirió tocar `proveedor_nombre`/`receptor_nombre` porque `computeDisplayCompanies()` ya resolvía el display correctamente vía coincidencia de CIF; auditado en `audit_logs`.
...
- Suite completa: 299 tests, 298 OK (mismo fallo preexistente de paridad v3, no relacionado). Cache-buster `admin-facturas.{js,css}` → `20260729-001`.
